...
export default function TestAttemptPage() {
  ...
  useEffect(() => {
    ...
    // Use ends_at if provided, otherwise compute from started_at + duration
    let endTimeMs;
    if (attempt.ends_at) {
      endTimeMs = new Date(attempt.ends_at).getTime();
    } else {
      const durationMs = (attempt.duration_minutes || 30) * 60 * 1000;
      const startTimeMs = new Date(attempt.started_at || attempt.created_at || new Date()).getTime();
    ...
    const timer = setInterval(() => {
      ...
      if (distance <= 0) {
        clearInterval(timer);
        setTimeLeft(0);
        forceSubmit('Auto-Submitted: Time Expired');
      } else {
  ...
  if (!q || questions.length === 0) {
    return (
      <div className="min-h-screen flex items-center justify-center p-6 bg-background">
        <Card p="xl" className="max-w-md w-full text-center bg-surface">
          <div className="w-20 h-20 bg-amber-500/10 text-amber-500 rounded-2xl flex items-center justify-center mx-auto mb-8">
            <AlertCircle size={40} />
          </div>
          <h2 className="text-2xl font-display font-extrabold text-text mb-4">No Questions Found</h2>
          <p className="text-text-muted font-sans mb-10">
            This assessment doesn't have any questions yet. The instructor may still be setting it up.
          </p>
          <Button 
            onClick={() => navigate('/student/dashboard')}
            variant="primary"
      ...
      <nav className="h-16 sm:h-20 bg-surface border-b border-border px-4 sm:px-8 flex items-center justify-between sticky top-0 z-50">
        ...
        <div className="flex items-center gap-3 sm:gap-6">
          <button 
            ...
            className="px-3 py-1.5 bg-brand/10 hover:bg-brand/20 text-brand rounded-lg text-xs font-bold transition-colors hidden sm:block"
          >
            Enter Fullscreen
          </button>
